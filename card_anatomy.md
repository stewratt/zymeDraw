...
| **Lift** | A rectangle of the piece lifts free and sets down elsewhere; the vacancy fills with the ground white — relocation only, nothing new enters | `liftSession.js` (`createLiftSession`) — marquee → floating pixels → click sets down; per-gesture undo | A thin behavior file (see `lift.jsx`); the fracture family (cards_plan §4) inherits the machinery |
