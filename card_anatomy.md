...
| **Smear** | The classic smudge: existing pixels drag and blend along the stroke. Nothing enters, nothing is coloured — the one brush that reads its own output | `smearSession.js` (`createSmearSession`) on `createStrokeEngine` — a pickup buffer the canvas and brush swap paint through, dab by dab | A thin behavior file (see `smear.jsx`, ~55 lines) |
