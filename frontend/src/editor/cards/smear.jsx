// Smear — the classic smudge brush, dragged across the whole piece. Nothing
...
// primitive underneath it (editor/smearSession.js — the pickup buffer). Its
// four hooks are the effect brushes' hooks with the reveal session swapped
// for the smear session: begin opens it, End drops the brush and leaves the
// overlay for the universal bake, Restart takes the overlay with it.

import { createSmearSession } from '../smearSession.js'
import { BrushSliders } from './maskControls.jsx'
import { CARD_TEXT } from '../cardText.js'
import { UI } from '../../copy/uiText.js'

export function beginSmear(ctx) {
  const controlsRef = { current: ctx.controls }
  const session = createSmearSession(ctx.canvas, {
    master: ctx.master,
...
export function updateSmear(ctx) {
...
export function commitSmear(ctx) {
  ctx.session?.session.dispose()
}

export function cleanupSmear(ctx) {
  if (!ctx.session) return
  ctx.session.session.dispose()
  ctx.session.session.removeOverlay()
}

export function SmearTools({ controls, info, ready, onControlChange }) {
  if (!ready) return <span className="hint">{UI.shared.preparing}</span>
  return (
    <div className="brush-tools card-tools">
      <p className="hint">{CARD_TEXT.smear.description}</p>
