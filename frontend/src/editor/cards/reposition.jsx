// Reposition — the whole canvas becomes one movable object for a round:
// drag, scale, rotate with the familiar handles, flip horizontal/vertical
// from the panel. Whatever hangs off the edge is gone at End; whatever the
// frame exposes is white. (Absorbs v1's Flip Canvas, with judgment added.)
//
...
export function beginReposition(ctx) {
...
export function updateReposition(ctx) {
  if (!ctx.session) return
  ctx.session.img.set({ flipX: !!ctx.controls.flipX, flipY: !!ctx.controls.flipY })
  ctx.canvas.requestRenderAll()
}

export function cleanupReposition(ctx) {
  if (!ctx.session) return
  ctx.canvas.remove(ctx.session.img)
  showMaster(ctx.canvas, ctx.session.master)
}

export function RepositionTools({ controls, ready, onControlChange }) {
  if (!ready) return <span className="hint">Preparing…</span>
  return (
    <div className="brush-tools card-tools">
      <p className="hint">
        The whole piece is in your hands for this round: drag, scale, rotate.
        What leaves the frame is gone; what the frame exposes stays white.
      </p>
