...
//
// EraseBrushControls is the brush block on its own, for cards whose brush
// is always in hand (Transfer — nothing to arrange, so no mode toggle).

export function EraseBrushControls({ controls, info, onControlChange }) {
  return (
    <>
...
export function ArrangeEraseControls({ controls, info, onControlChange }) {
  ...
  return (
    <>
      ...
      {erasing && <EraseBrushControls controls={controls} info={info} onControlChange={onControlChange} />}
    </>
  )
}
